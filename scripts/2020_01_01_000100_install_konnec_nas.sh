...
composer install --optimize-autoloader --working-dir=/var/www/konnec_nas
...
cp ~/konnec_nas/setup/konnec-nas.conf /etc/apache2/sites-available
a2ensite konnec-nas.conf
systemctl restart apache2

#Supervisor
echo "Configuring Supervisor"
cp /var/www/konnec_nas/laravel-worker.conf /etc/supervisor/conf.d
# cp /var/www/konnec_nas/echo-server-worker.conf /etc/supervisor/conf.d
supervisorctl reread
supervisorctl update
supervisorctl start laravel-worker:*
# supervisorctl start echo-server-worker:*